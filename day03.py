...
def get_items_priority(misplaced_items: List[str]) -> List[int]:
    priority = {chr(key): value + 1 for (value, key) in enumerate(itertools.chain(range(ord('a'), ord('z')+1), range(ord('A'), ord('Z')+1)))}
...
if __name__ == "__main__":
    ...
    misaplece_items = get_misplaced_items(puzzle_input)
    misplaced_items_priority = get_items_priority(misaplece_items)
